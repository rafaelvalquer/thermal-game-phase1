...
test('air handler cools supply air and condenser rejects cooling plus electrical work',()=>{
  ...
  const handler=new AirHandler(2,3,{maxAirFlow:1,coolingCapacity:10000}),condenser=new Condenser(0,0,{coolingCapacity:15000});
  const supplyVent=new SupplyVent(5,3),returnVent=new ReturnVent(2,6);
  world.addEntity(handler);world.addEntity(condenser);world.addEntity(supplyVent);world.addEntity(returnVent);
  ...
  assert.ok(Math.abs(handler.actualRoomCooling-handler.coolingPower)/handler.coolingPower<.05);
  ...
  assert.ok(world.temperatureAt(returnVent.x,returnVent.y)<35);
  assert.ok(metrics.externalEnergy>0);assert.ok(Math.abs(metrics.energyBalance)<1e-3);
});

test('direct room return balances an HVAC network without a return vent',()=>{
  const world=new World(9,7),metrics={generatedHeat:0,externalEnergy:0,powerDraw:0,powerEnergy:0,energyBalance:0};
  const handler=new AirHandler(1,3,{maxAirFlow:1,coolingCapacity:15000}),condenser=new Condenser(0,0,{coolingCapacity:20000}),vent=new SupplyVent(5,3);
  world.addEntity(handler);world.addEntity(condenser);world.addEntity(vent);world.addUtility(new AirDuct(2,3));world.addUtility(new AirDuct(3,3));world.addUtility(new AirDuct(4,3));
  ...
  assert.equal(handler.status,'DIRECT ROOM RETURN');assert.ok(handler.currentFlow>0);assert.ok(handler.actualRoomCooling>0);
});

test('hot outdoor conditions lower COP and can trigger high head status',()=>{
  const world=new World(9,7),metrics={generatedHeat:0,externalEnergy:0,powerDraw:0,powerEnergy:0,energyBalance:0};
  world.environment.temperature=35;
  const handler=new AirHandler(1,3,{maxAirFlow:1,coolingCapacity:25000}),condenser=new Condenser(0,0,{coolingCapacity:30000}),vent=new SupplyVent(5,3),returns=new ReturnVent(1,6);
  world.addEntity(handler);world.addEntity(condenser);world.addEntity(vent);world.addEntity(returns);
  for(const [x,y,role] of [[2,3,'supply'],[3,3,'supply'],[4,3,'supply'],[1,4,'return'],[1,5,'return']])world.addUtility(new AirDuct(x,y,{role}));
...
test('indoor condenser returns rejected heat to the room instead of outdoor accounting',()=>{
  ...
  const handler=new AirHandler(2,3,{maxAirFlow:1,coolingCapacity:10000}),condenser=new Condenser(0,0,{coolingCapacity:15000});
  world.addEntity(handler);world.addEntity(condenser);world.addEntity(new SupplyVent(5,3));world.addEntity(new ReturnVent(2,6));
  world.addUtility(new AirDuct(3,3));world.addUtility(new AirDuct(4,3));
  world.addUtility(new AirDuct(2,4,{role:'return'}));world.addUtility(new AirDuct(2,5,{role:'return'}));
  for(let y=4;y<=7;y++)for(let x=1;x<=3;x++)world.setTemperature(x,y,35);
  const airflow=new AirflowSystem(world,metrics),hvac=new HVACSystem(world,airflow,metrics),energy=new EnergySystem(world,metrics);energy.initialize();
