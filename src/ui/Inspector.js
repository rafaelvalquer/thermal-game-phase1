...
export class Inspector {
  ...
  entity(world,e){
    ...
    if(e.type==='machine'){
      ...
      rows.push(['Temperatura',e.temperature.toFixed(2)+' °C'],['Geração',formatPower(e.heatOutput)],['Resfriamento',formatPower(e.coolingPower)],['Operação',e.started?'Ligada':'Aguardando']);
    }
    ...
    const state=temperature!=null?thermalState(temperature):{id:'stable',label:e.enabled?'OPERACIONAL':'DESLIGADO',color:'#34d399'};
  ...
  symbol(type){return ({machine:'▣',fan:'✣',exhaust:'◉',pipe:'━',pump:'⟳',tank:'▰',radiator:'▥',exchanger:'HX',sensor:'°'}[type]||'□');}
}
