...
export class World {
  ...
  massAt(x,y) {
    ...
    return Math.max(0.0001, m.density * TILE_VOLUME);
  }
  capacityAtIndex(i) {
    const m = this.registry.fromIndex(this.material[i]);
    return Math.max(0.001, m.density * TILE_VOLUME * m.heatCapacity);
  }
  ...
  setMaterial(x,y,id,{preserveTemperature=true}={}) {
    ...
    const i=this.index(x,y); const t=preserveTemperature ? this.temperatureAtIndex(i) : OUTDOOR_TEMP;
    this.material[i] = this.registry.index(id); this.energy[i] = this.capacityAtIndex(i) * t; return true;
  }
