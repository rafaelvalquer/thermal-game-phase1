...
export class AirflowSystem {
  ...
  buildVelocityField(){
    ...
    for(const e of w.entities){
      ...
      for(let step=0;step<=e.range;step++){
        ...
        for(let side=-2;side<=2;side++){
          const sx=cx+(dir.y!==0?side:0),sy=cy+(dir.x!==0?side:0);
    ...
    this.applyRadiatorNaturalConvection();
  }

  applyRadiatorNaturalConvection(){
    const w=this.world;
    for(const radiator of w.entitiesByType('radiator')){
      ...
      if(delta<=.5)continue;
      const strength=(radiator.naturalAirflow||.28)*clamp(delta/20,0,1);

      for(let dy=-2;dy<=2;dy++)for(let dx=-2;dx<=2;dx++){
        const x=radiator.x+dx,y=radiator.y+dy,d=Math.hypot(dx,dy);
        if(d<.5||d>2.5||!w.inBounds(x,y)||!w.isAir(x,y))continue;
        const i=w.index(x,y),fall=1/(1+d);
        w.airX[i]+=(dx/d)*strength*fall;
        w.airY[i]+=(dy/d)*strength*fall;
      }
    }
  }
  ...
  advectHeat(dt){
    ...
    for(let y=1;y<w.height-1;y++)for(let x=1;x<w.width-1;x++){
      ...
      const j=w.index(nx,ny),Ts=w.temperatureAtIndex(i),Td=w.temperatureAtIndex(j),dT=Ts-Td;if(Math.abs(dT)<1e-5)continue;
  ...
  applyExhaust(dt){
    ...
    for(const e of w.entitiesByType('exhaust')){
      ...
      const massFlow=airDensity*faceArea*e.airflow*efficiency;
      ...
      for(const c of cells){
        ...
        const localMassFlow=massFlow*(c.weight/weightSum),fraction=clamp((localMassFlow*dt)/cellMass,0,.65),q=(T-out)*cap*fraction;
