...
export class FluidSystem {
  ...
  assignFlow(net){
    const pumps=net.filter(e=>e.type==='pump'&&e.enabled),resistance=net.reduce((s,e)=>s+(e.resistance||1),0)||1;
    const drive=pumps.reduce((s,p)=>s+p.hydraulicPower,0),flow=clamp(drive/resistance,0,5);net.forEach(e=>e.flowRate=flow);
  }
  transport(net,dt){
    if(net.length<2)return;const byPos=new Map(net.map(e=>[keyOf(e.x,e.y),e])),done=new Set();
  ...
  circulate(net,dt){
    ...
    if(capacity<=0)return;const target=energy/capacity,fraction=clamp(flow*dt*1.5,0,.22);
  ...
  exchangeMachines(dt){
    ...
    for(const ex of this.world.entitiesByType('exchanger')){
      ...
      const m=machines.filter(m=>manhattan(m,ex)<=1).sort((a,b)=>b.temperature-a.temperature)[0];if(!m)continue;
