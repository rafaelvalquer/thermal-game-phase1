...
export class CoolingAirExchange {
  ...
  transferMassEnergy(vent,dt){
    ...
    const i=this.world.index(vent.x,vent.y),roomT=this.world.temperatureAtIndex(i),capacity=this.world.capacityAtIndex(i),mass=this.world.massAt(vent.x,vent.y);
    const fraction=clamp((vent.flowRate*COOLING.airDensity*dt)/Math.max(mass,1e-6),0,COOLING.maxExchangeFraction),energyDelta=capacity*fraction*(vent.airTemperature-roomT);
    this.world.energy[i]+=energyDelta;return energyDelta/dt;
  }
