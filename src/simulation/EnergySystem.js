export class EnergySystem {
  ...
  totalInternalEnergy(){
    ...
    for(const e of this.world.entities){ if(e.type==='machine'||['pipe','pump','tank','radiator','exchanger'].includes(e.type)) total+=e.energy||0; }
    return total;
  }
  ...
  update(dt){
    ...
    for(const e of this.world.entities){
      if(!e.enabled||!e.power) continue; power+=e.power;
      const waste=e.power*(e.wasteHeatFraction??0)*dt; if(waste>0){
        if(['pump'].includes(e.type) && typeof e.energy==='number') e.energy+=waste;
